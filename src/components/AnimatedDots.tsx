...
export const AnimatedDots: React.FC<AnimatedDotsProps> = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      createDots();
      // Only clear canvas on resize to avoid trailing issues when window changes
      ctx.clearRect(0, 0, width, height);
    };

    const createDots = () => {
      dotsRef.current = [];
      // Calculate how many dots we need to fill the screen width
      const actualDotsNum = fullScreen ? Math.ceil(width / (dotRadius * 2 + dotSpacing)) : dotsNum;
      for (let i = 0; i < actualDotsNum; i++) {
